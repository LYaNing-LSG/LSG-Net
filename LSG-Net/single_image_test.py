...
def load_image(img_path, mask_path,edge_path, sigma256=3.0):
    ...
    edge_256 =  cv2.imread(edge_path)
    edge = cv2.resize(edge_256,(512,512))
    edge = cv2.cvtColor(edge,cv2.COLOR_BGR2GRAY)
    ret,edge = cv2.threshold(edge,120,255,THRESH_BINARY)
    edge_256 = rgb2gray(edge_256)
...
def test(model, wf, img_path, mask_path,edge_path, save_path, valid_th, sigma256=3.0):
    items = load_image(img_path, mask_path, edge_path,sigma256)
    ...
    items['line_256'] = line
    line_512 = wf_inference_test(wf, items['img_512'].cuda(), h=512, w=512, masks=items['mask_512'].cuda(),
                             valid_th=valid_th, mask_th=valid_th)
    items['line'] = line_512
    ...
    with torch.no_grad():
        ...
        edge_pred, line_pred = SampleEdgeLineLogits(model.inpaint_model.transformer,
                                                    context=[items['img_256'], items['edge_256'],items['edge'], items['line_256'],items['line']],
                                                    mask=items['mask_256'].clone(), mask_L = items['mask'].clone(),iterations=5,
        ...
        line_pred = torch.sigmoid(line_pred)
        edge_pred[edge_pred > 0.6] = 1
        edge_pred[edge_pred <= 0.6] = 0
        line_pred[line_pred > 0.6] = 1
        line_pred[line_pred <= 0.6] = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument('--edge_path', type=str, help='test mask path')
    ...
    test(model, wf, args.img_path, args.mask_path,args.edge_path, args.save_path, 0.85, sigma256=3.0)
    e_time = time.time()
