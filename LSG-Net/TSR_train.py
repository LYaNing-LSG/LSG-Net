...
def main_worker(rank, opts):
    ...
    # Define the dataset
    if not opts.MaP:
        train_dataset = ContinuousEdgeLineDatasetMask(opts.data_path, edge_path=opts.edge_path,irregular_mask_path=opts.irregular_mask_path,
                                                      coco_mask_path=opts.coco_mask_path,is_train=True,
                                                      mask_rates=opts.mask_rates, image_size=opts.image_size,
                                                      line_path=opts.train_line_path)
        test_dataset = ContinuousEdgeLineDatasetMask(opts.validation_path,edge_path=opts.val_edge_path, test_mask_path=opts.valid_mask_path,
                                                     is_train=False, image_size=opts.image_size,
                                                     line_path=opts.val_line_path)
    else:
        train_dataset = ContinuousEdgeLineDatasetMaskFinetune(opts.data_path,edge_path=opts.edge_path, irregular_mask_path=opts.irregular_mask_path,
                                                              coco_mask_path=opts.coco_mask_path,is_train=True,
                                                              mask_rates=opts.mask_rates, image_size=opts.image_size,
                                                              line_path=opts.train_line_path)
        test_dataset = ContinuousEdgeLineDatasetMaskFinetune(opts.validation_path,edge_path=opts.val_edge_path, test_mask_path=opts.valid_mask_path,
                                                             is_train=False, image_size=opts.image_size,
                                                             line_path=opts.val_line_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--edge_path', type=str, default=None, help='Indicate where is the edge set')
    parser.add_argument('--train_line_path', type=str, default=None, help='Indicate where is the wireframes of training set')
    parser.add_argument('--irregular_mask_path', type=str, default=None)
    parser.add_argument('--coco_mask_path', type=str, default=None)
    ...
    parser.add_argument('--val_edge_path', type=str, default=None)
