    ...
    def train(self, loaded_ckpt):
        ...
        for epoch in range(config.max_epochs):
            ...
            for it, items in enumerate(tqdm(loader, disable=not_show_tqdm)):
                ...
                edge, line, loss,edge_loss, line_loss = model(items['img'], items['edge'], items['line'], items['edge'], items['line'],
                                         items['mask'])
                loss = loss.mean()  # collapse all losses if they are scattered on multiple gpus
                losses.append(loss.item())
                if self.global_rank == 0:
                    writer.add_scalar('Loss/Total_Loss', loss.item(), self.iterations)
                    writer.add_scalar('Loss/Edge_Loss', edge_loss.item(), self.iterations)
                    writer.add_scalar('Loss/Line_Loss', line_loss.item(), self.iterations)
                ...
                if it % self.config.print_freq == 0 and self.global_rank == 0:
                    self.logger.info(
                        f"epoch {epoch + 1} iter {it}/{self.iterations_per_epoch}: train loss {loss.item():.5f}. edge loss {edge_loss.item():.5f}. line loss {line_loss.item():.5f}. lr {lr:e}")
        ...
        for it, items in enumerate(tqdm(dataloader, disable=False)):
            ...
            for k in items:
                if type(items[k]) is torch.Tensor:
                    items[k] = items[k].to(self.device)
            with torch.no_grad():
                edge, line, _,_,_ = model(items['img'], items['edge'], items['line'], masks=items['mask'])
    ...
    def train(self, loaded_ckpt):
        ...
        for epoch in range(config.max_epochs):
            ...
            for it, items in enumerate(tqdm(loader, disable=not_show_tqdm)):
                ...
                edge, line, loss,edge_loss, line_loss = model(items['mask_img'], items['edge'], items['line'], items['edge'], items['line'],
                                         items['erode_mask'])
                loss = loss.mean()  # collapse all losses if they are scattered on multiple gpus
                losses.append(loss.item())
                # if self.global_rank == 0:
                ...
                if self.iterations % 2000 == 1 and self.global_rank == 0:
                    ...
                    # supports early stopping based on the test loss, or just save always if no test set is provided
                    good_model = self.test_dataset is None or average_F1 >= bestAverageF1
                    if self.EdgeLineGPT256RelBCE.ckpt_path is not None and good_model and self.global_rank == 0:  ## Validation on the global_rank==0 process
                        bestAverageF1 = average_F1
                        EdgeF1 = edge_F1
                        LineF1 = line_F1
                        self.logger.info("current best epoch is %d" % (epoch))
