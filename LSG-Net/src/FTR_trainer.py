...
class ZITS:
    def __init__(self, config, gpu, rank, test=False, single_img_test=False):
        ...
        if not test:
            self.train_dataset = DynamicDataset(config.TRAIN_FLIST, batch_size=config.BATCH_SIZE // config.world_size,
                                                edge_path=config.EDGE_FLIST,irregular_mask_path = config.IRREGULAR_MASK_FLIST,coco_mask_path=config.COCO_MASK_FLIST,
        ...
        if not single_img_test:
            self.val_dataset = DynamicDataset(config.VAL_FLIST, batch_size=config.BATCH_SIZE,edge_path=config.EDGE_VAL_FLIST,irregular_mask_path=None,coco_mask_path=None, pos_num=config.rel_pos_num,
    ...
    def train(self):
        ...
        while keep_training:
            ...
            for _, items in enumerate(train_loader):
                ...
                items['edge'] = F.interpolate(items['edge'] , size=(image_size, image_size), mode='bilinear',
                                                  align_corners=False)
                ...
                if iteration > int(self.config.MIX_ITERS):
                    ...
                    edge_pred, line_pred = SampleEdgeLineLogits(self.inpaint_model.transformer,
                                                                context=[items['img_256'][:b, ...],
                                                                         ...
                                                                         items['line_256'][:b, ...],
                                                                        items['line'][:b, ...]],
                                                                mask=items['mask_256'][:b, ...].clone(),mask_L=items['mask'][:b, ...].clone(),
    ...
    def sample(self, it=None):
        ...
        with torch.no_grad():
            ...
            line_pred[line_pred >= 0.5] = 1
            line_pred[line_pred < 0.5] = 0
