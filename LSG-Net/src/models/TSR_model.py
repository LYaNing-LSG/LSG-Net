...
from .transformer import Block, my_Block_2
...
class EdgeLineGPT256RelBCE(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.blocks = Block(dim=ch*4,order=5)
        self.my_block = my_Block_2(config,ch)
        self.blocks_1 = Block(dim=ch*4,order=6)
        self.my_block_1 = my_Block_2(config,ch)
        self.blocks_2 = Block(dim=ch*4,order=5)
        self.my_block_2 = my_Block_2(config,ch)
        self.blocks_3 = Block(dim=ch*4,order=6)
        self.my_block_3 = my_Block_2(config,ch)
    ...
    def forward(self, img_idx, edge_idx, line_idx, edge_targets=None, line_targets=None, masks=None):
        ...
        x = self.blocks(x)
        x = self.my_block(x,masks)
        x = self.blocks_1(x)
        x = self.my_block_1(x,masks)
        x = self.blocks_2(x)
        x = self.my_block_2(x,masks)
        x = self.blocks_3(x)
        x = self.my_block_3(x,masks)
        x = x.permute(0, 2, 3, 1)
        ...
        if edge_targets is not None and line_targets is not None:
            ...
            line_loss = torch.mean(line_loss)
        else:
            loss = 0
            edge_loss = 0
            line_loss = 0
        edge, line = self.act_last(edge), self.act_last(line)
        return edge, line, loss, edge_loss, line_loss
